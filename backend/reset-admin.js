...
db.get("SELECT id FROM users WHERE username = 'admin'", (err, row) => {
    ...
    if (!row) {
        ...
        db.run(
            `INSERT INTO users 
            (username, password, full_name, email, role, is_active, can_report_lab_tickets)
            VALUES 
            ('admin', 'Admin123!', 'Administrador del Sistema', 'admin@ticketfast.com', 'admin', 1, 1)`,
            function (err) {
                if (err) {
        ...
        db.run(
            ...
            function (err) {
                if (err) {
                    ...
                    console.log(
                        '✅ Contraseña actualizada. Filas afectadas:',
                        this.changes
                    );
                    console.log('🔑 Contraseña: Admin123!');
                }
